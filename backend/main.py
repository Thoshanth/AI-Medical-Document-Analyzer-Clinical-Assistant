...
from backend.evaluation.eval_pipeline import (
    run_full_evaluation,
    get_saved_results,
)
from backend.evaluation.ragas_evaluator import run_ragas_evaluation
from backend.evaluation.medical_metrics import evaluate_emergency_detection
...
# ══════════════════════════════════════════════════════════════════
# STAGE 11 — Clinical Evaluation
# ══════════════════════════════════════════════════════════════════

@app.post("/evaluation/run/{document_id}", tags=["Stage 11 - Evaluation"])
def run_evaluation(document_id: int):
    """
    Run complete medical AI evaluation.

    Phase 1: Build golden evaluation dataset
    Phase 2: RAGAS metrics
      - Faithfulness (anti-hallucination)
      - Answer relevancy
      - Context precision
      - Context recall
    Phase 3: Medical-specific metrics
      - Emergency detection accuracy
      - Disclaimer compliance (must be 100%)
      - Clinical accuracy (LLM-judged)
      - Medication safety language

    Returns overall grade + production readiness assessment.
    Requires Stage 4 RAG to be indexed first.
    """
    logger.info(f"Full evaluation | doc_id={document_id}")
    try:
        return run_full_evaluation(document_id)
    except Exception as e:
        logger.error(f"Evaluation failed: {e}", exc_info=True)
        raise HTTPException(500, str(e))


@app.post("/evaluation/emergency-test", tags=["Stage 11 - Evaluation"])
def test_emergency_detection():
    """
    Test emergency detection accuracy specifically.

    Runs 5 test cases (3 emergencies + 2 normal queries).
    Measures sensitivity and specificity.
    Sensitivity must be >= 0.95 for patient safety.
    """
    try:
        return evaluate_emergency_detection()
    except Exception as e:
        logger.error(f"Emergency test failed: {e}", exc_info=True)
        raise HTTPException(500, str(e))


@app.post("/evaluation/build-dataset/{document_id}", tags=["Stage 11 - Evaluation"])
def build_eval_dataset(document_id: int, include_static: bool = True):
    """
    Build golden evaluation dataset for a document.
    Combines 8 static medical QA pairs with
    document-specific pairs generated by MiniMax.
    """
    try:
        return build_golden_dataset(document_id, include_static)
    except Exception as e:
        logger.error(f"Dataset build failed: {e}", exc_info=True)
        raise HTTPException(500, str(e))


@app.get("/evaluation/results/{document_id}", tags=["Stage 11 - Evaluation"])
def get_evaluation_results(document_id: int):
    """
    Retrieve previously saved evaluation results.
    No LLM calls — reads from saved report file.
    """
    results = get_saved_results(document_id)
    if not results:
        raise HTTPException(
            404,
            f"No evaluation found for document {document_id}. "
            f"Run POST /evaluation/run/{document_id} first."
        )
    return results